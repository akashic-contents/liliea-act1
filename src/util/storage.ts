// cf. https://developer.mozilla.org/ja/docs/Glossary/Base64

export function toSaveDataString<T>(saveData: T): string {
	let saveDataString = "";
	try {
		saveDataString = JSON.stringify(saveData);
	} catch (error) {
		console.warn("Invalid saveData:", error);
		throw error;
	}

	return base64EncArr(strToUTF8Arr(saveDataString));
}

export function toSaveData<T>(saveDataString: string): T {
	const decoded = UTF8ArrToStr(base64DecToArr(saveDataString));
